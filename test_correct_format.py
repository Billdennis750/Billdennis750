#!/usr/bin/env python3
"""
Test Xixapay with correct field formats
"""

import asyncio
import httpx
import json
from dotenv import load_dotenv
load_dotenv('/app/backend/.env')
import sys
sys.path.append('/app/backend')
from config import get_settings

async def test_correct_format():
    """Test with correct field formats"""
    settings = get_settings()
    
    print("✅ Testing Xixapay with Correct Format")
    print("=" * 45)
    
    headers = {
        "api-key": settings.xixapay_api_key,
        "Authorization": f"Bearer {settings.xixapay_public_key}",
        "Content-Type": "application/json"
    }
    
    async with httpx.AsyncClient(timeout=30.0) as client:
        
        print("1. Creating customer with correct format...")
        try:
            customer_payload = {
                "first_name": "John",
                "last_name": "Doe",
                "email": "john.doe@example.com",
                "phone_number": "08012345678"  # 11 digits Nigerian format
            }
            
            response = await client.post(
                f"{settings.xixapay_base_url}/api/customer/create",
                headers=headers,
                json=customer_payload
            )
            print(f"   Status: {response.status_code}")
            print(f"   Response: {response.text}")
            
            if response.status_code == 200:
                customer_data = response.json()
                print("   ✅ Customer created successfully!")
                
                # Extract customer ID
                customer_id = customer_data.get("data", {}).get("id") or customer_data.get("id")
                print(f"   Customer ID: {customer_id}")
                
                # Now test virtual account creation
                print("\n2. Testing virtual account creation...")
                
                va_payloads = [
                    {
                        "customer_id": customer_id,
                        "amount": 2500,
                        "description": "Loan Processing Fee"
                    },
                    {
                        "customer_email": "john.doe@example.com",
                        "amount": 2500,
                        "description": "Loan Processing Fee",
                        "reference": "CASHFLOW-TEST-123"
                    },
                    {
                        "first_name": "John",
                        "last_name": "Doe", 
                        "email": "john.doe@example.com",
                        "phone_number": "08012345678",
                        "amount": 2500,
                        "description": "Loan Processing Fee"
                    }
                ]
                
                va_endpoints = [
                    "/api/virtual-account/create",
                    "/api/virtual-account/dynamic/create",
                    "/api/v1/virtual-account/create"
                ]
                
                for i, va_payload in enumerate(va_payloads):
                    for endpoint in va_endpoints:
                        try:
                            print(f"\n   Testing {endpoint} with payload {i+1}...")
                            va_response = await client.post(
                                f"{settings.xixapay_base_url}{endpoint}",
                                headers=headers,
                                json=va_payload
                            )
                            print(f"      Status: {va_response.status_code}")
                            print(f"      Response: {va_response.text[:300]}...")
                            
                            if va_response.status_code == 200:
                                print(f"      ✅ Virtual account created!")
                                va_data = va_response.json()
                                return va_data
                                
                        except Exception as e:
                            print(f"      Error: {str(e)}")
                
            else:
                print(f"   ❌ Customer creation failed")
                
        except Exception as e:
            print(f"   Error: {str(e)}")

if __name__ == "__main__":
    asyncio.run(test_correct_format())